...
export async function getProfileByUserId(
  ...
  const { data, error } = await supabase
    .from("profiles")
    .select(
      "id, full_name, company_name, company_phone, company_email, company_license, company_address, company_logo_url, company_theme_color, default_markup_percent, default_tax_percent, onboarding_completed, country, region_code, company_city, company_postal, tax_id, selected_trades, currency, measurement_units, created_at, updated_at",
    )
    .eq("id", userId)
    .maybeSingle();
  if (error) {
